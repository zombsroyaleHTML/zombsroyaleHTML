function createUnityInstance(t, r, d) {
    ...
    function n(e) {
        ...
        (r += "\n" + (n = n.startsWith(r) ? n.substring(r.length) : n).trim()) && c.stackTraceRegExp && c.stackTraceRegExp.test(r) && C(r, e.filename || t && (t.fileName || t.sourceURL) || "", e.lineno || t && (t.lineNumber || t.line) || 0)
    }
    function e(e, t, r) {
        var n = e[t];
    ...
    ;
    var u, l, h, f, p, g, m, b, y = "", v = "", w = (document.addEventListener("webkitfullscreenchange", function(e) {
        document.webkitCurrentFullScreenElement === t ? t.style.width && (y = t.style.width,
        v = t.style.height,
        t.style.width = "100%",
        t.style.height = "100%") : y && (t.style.width = y,
        t.style.height = v,
        v = y = "")
    }),
    {
    ...
    function x(e, t) {
        if ("symbolsUrl" != e) {
            ...
            for (e in c.downloadProgress) {
                if (!(r = c.downloadProgress[e]).started)
                    return;
                a++,
        ...
        o.isConnected = new Promise(function(t, r) {
            try {
                ...
                e.onupgradeneeded = function(e) {
                    ...
                    e.objectStoreNames.contains(l.name) || (t = e.createObjectStore(l.name, {
                        keyPath: "url"
                    }),
    ...
    function E(e) {
        return E.link = E.link || document.createElement("a"),
        E.link.href = e,
        E.link.href
    }
    function k(t) {
                ...
                if ("undefined" == typeof unityFramework || !unityFramework) {
                    ...
                    for (e in t) {
                        ...
                        if (c.frameworkUrl.endsWith("." + n[0]))
                            return r = "Unable to parse " + c.frameworkUrl + "!",
                            "file:" == location.protocol ? void i(r + " Loading pre-compressed (brotli or gzip) content via a file:// URL without a web server is not supported by this browser. Please use a local development web server to host compressed Unity content, or use the Unity Build and Run option.", "error") : (r += ' This can happen if build compression was enabled but web server hosting the content was misconfigured to not serve the file with HTTP Response Header "Content-Encoding: ' + n[1] + '" present. Check browser Console and Devtools Network tab to debug.',
        ...
        c.preRun.push(function() {
            ...
            o.then(function(e) {
                ...
                for (r += 4; r < o; ) {
                    ...
                    r += i;
                    for (var u = 0, l = d.indexOf("/", u) + 1; 0 < l; u = l,
                    l = d.indexOf("/", u) + 1)
    ...
    return c.SystemInfo = function() {
        var e, t, r, n, o = navigator.userAgent + " ", a = [["Firefox", "Firefox"], ["OPR", "Opera"], ["Edg", "Edge"], ["SamsungBrowser", "Samsung Browser"], ["Trident", "Internet Explorer"], ["MSIE", "Internet Explorer"], ["Chrome", "Chrome"], ["CriOS", "Chrome on iOS Safari"], ["FxiOS", "Firefox on iOS Safari"], ["Safari", "Safari"]];
        function s(e, t, r) {
        ...
        var l, c = {
            ...
            "NT 6.3": "8.1",
            "NT 10.0": "10"
        }[c] || c, h = ((h = document.createElement("canvas")) && (l = (f = h.getContext("webgl2")) ? 2 : 0,
        f || (f = h && h.getContext("webgl")) && (l = 1),
        f && (n = f.getExtension("WEBGL_debug_renderer_info") && f.getParameter(37446) || f.getParameter(7937))),
        "undefined" != typeof SharedArrayBuffer), f = "object" == typeof WebAssembly && "function" == typeof WebAssembly.compile;
        return {
            width: screen.width,
        ...
        function o() {
            return void 0 === e ? a.arrayBuffer().then(function(e) {
                ...
                if (e.done) {
                    ...
                    if (h < u)
                        return l.slice(0, h);
                    for (var t = new Uint8Array(h), r = (t.set(l, 0),
                    f), n = 0; n < c.length; ++n)
                        t.set(c[n], r),
                        r += c[n].length;
                    return t
                ...
                f = h + e.value.length) : c.push(e.value),
                h += e.value.length,
                s({
                    type: "progress",
                    response: a,
    ...
    S.prototype.execute = function(a, s, i) {
        return this.isConnected.then(function() {
            return new Promise(function(t, r) {
                try {
                    var e, n, o;
                    null === this.database ? r(new Error("indexedDB access denied")) : (e = -1 != ["put", "delete", "clear"].indexOf(s) ? "readwrite" : "readonly",
                    n = this.database.transaction([a], e).objectStore(a),
                    "openKeyCursor" == s && (n = n.index(i[0]),
                    i = i.slice(1)),
    ...
    ,
    function(n, o) {
        var e, t, a = g.getInstance(), r = E("string" == typeof n ? n : n.url), s = {
            enabled: (e = r,
            (!(t = o) || !t.method || "GET" === t.method) && ((!t || -1 != ["must-revalidate", "immutable"].indexOf(t.control)) && !!e.match("^https?://")))
        };
        function i(e, t) {
            return b(e, t).then(function(e) {
                if (s.enabled && !s.revalidated) {
                    if (304 === e.status)
                        return s.result.revalidated = s.result.accessed,
                        s.revalidated = !0,
                        a.storeRequest(s.result).then(function() {
                    ...
                    200 == e.status ? (s.result = P(e.url, s.company, s.product, s.accessed, e),
                    s.revalidated = !0,
                    a.storeRequest(s.result).then(function() {
        ...
        return o && (s.control = o.control,
        s.company = o.company,
        s.product = o.product),
        s.result = P(r, s.company, s.product, Date.now()),
        s.revalidated = !1,
        s.enabled ? a.loadRequest(s.result.url).then(function(e) {
            if (!e || e.version !== m.version)
                return i(n, o);
            s.result = e,
            s.result.accessed = Date.now();
            var t, r = new k(s.result.response);
            return "immutable" == s.control ? (s.revalidated = !0,
            a.storeRequest(s.result),
            T("'" + s.result.url + "' served from the indexedDB cache without revalidation"),
            d(r),
            r) : (e = s.result.url,
            (t = window.location.href.match(/^[a-z]+:\/\/[^\/]+/)) && !e.lastIndexOf(t[0], 0) || !r.headers.get("Last-Modified") && !r.headers.get("ETag") ? (e = (o = o || {}).headers || {},
            o.headers = e,
            r.headers.get("Last-Modified") ? (e["If-Modified-Since"] = r.headers.get("Last-Modified"),
            e["Cache-Control"] = "no-cache") : r.headers.get("ETag") && (e["If-None-Match"] = r.headers.get("ETag"),
            e["Cache-Control"] = "no-cache"),
            i(n, o)) : fetch(s.result.url, {
                method: "HEAD"
            }).then(function(t) {
                return s.revalidated = ["Last-Modified", "ETag"].every(function(e) {
                    return !r.headers.get(e) || r.headers.get(e) == t.headers.get(e)
                }),
                s.revalidated ? (s.result.revalidated = s.result.accessed,
                a.storeRequest(s.result),
                T("'" + s.result.url + "' successfully revalidated and served from the indexedDB cache"),
                d(r),
                r) : i(n, o)
            ...
            return T("Failed to load '" + s.result.url + "' from indexedDB cache due to the error: " + e),
            b(n, o)
        }) : b(n, o)
    }
    ),
